...
def autoprocess():
   ...
   for i,vals in enumerate(to_process):
      ...
      for k,col in enumerate(data):
         if (data.dtypes[k] != 'object') and (data.dtypes[k] != 'bool'):
            try:            
               fig,ax = plt.subplots(nrows=1,ncols=1,figsize=(8,8))
               ax.plot([idx for idx,j in enumerate(data[col])],array(data[col]))
               ax.set_xlabel(col)
               ax.set_title('Autogenerated Plot ' + str(k + 1))
               ax.set_facecolor('#ADD8E6')
               ax.set_axisbelow(True)
               ax.yaxis.grid(color='white', linestyle='-')
               ax.xaxis.grid(color='white', linestyle='-')
               savepath = os.path.join(plots_path,(filename+'_Plot-'+str(k + 1)+'.png'))
               fig.savefig(savepath,format='png')
               plt.close('all')
            except:
               plt.close('all')
